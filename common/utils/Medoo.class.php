...
class Medoo
{
				...
				if ($match[2] != '' && $match[3] != '')
				{
					...
					if (is_array($relation))
					{
						...
						else
						{
							...
							$joinTable = $table;
							$joinFiled =  key($relation);
							$joinFiledExp = explode('.', $joinFiled);
							if(count($joinFiledExp)>1){
								$joinTable = $joinFiledExp[0];
								$joinTable = '"'.$joinTable.'"';
								$joinFiled = $joinFiledExp[1];
							}
							
							$relation = 'ON ' . $joinTable . '."' . $joinFiled . '" = "' . $match[3] . '"."' . current($relation) . '"';
						}
					}
	...
	public function insert($table, $datas)
	{
		...
		foreach ($datas as $data)
		{
			...
			$execResult = $this->exec('INSERT INTO "' . $table . '" (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')');
			if(empty($execResult)){
				$execResult = false;
			}
